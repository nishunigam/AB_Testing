...
1. Invariant (Sanity) CheckBefore choosing the measurement metrics that will be affected by the variants, we need to make sure invariants are controlled. For example, do you have the same number of users across the two groups (population sizing)? Do they have comparable distributions across countries, languages? Ideally, we should also assign the two groups the same number of trials.

2. Determine High-level Business MetricsThis usually involves discussing with domain experts to determine the best practice, such as revenues, market shares, etc. In our online learning case, click-through probability makes more business sense.

3. Expand to More Detailed MetricsFirst of all, we could list the basic customer funnel and expand it with more details to better understand the data we need to collect for the metrics. In this case, we need to capture the number of unique users that moves from "exploring the site" to "create an account."
...
Ethical issuesSecurity and confidentiality issues; Whether informed consent is feasible.
